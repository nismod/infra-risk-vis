...
if __name__ == '__main__':
    ...
    for idx, batch in enumerate(
        ...
        csv_path = fname.replace(".geoparquet", f"_{network_tile_layer.layer}_features_{idx}.csv")
        df["geom"] = ""
        df[feature_columns].to_csv(csv_path, index=False)

        df_rp_damage = parse_rp_damage_batch(df, primary_key_column="id")
        rp_csv_path = fname.replace(".geoparquet", f"_{network_tile_layer.layer}_rp_{idx}.csv")
        df_rp_damage.to_csv(rp_csv_path, index=False)

        df_expected_damage = parse_exp_damage_batch(df, primary_key_column="id")
        exp_csv_path = fname.replace(".geoparquet", f"_{network_tile_layer.layer}_exp_{idx}.csv")
